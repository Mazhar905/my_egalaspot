      ...
      <footer className="bg-gray-800 pt-5 font-sans tracking-wide">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 px-10 py-8">
          <div>
            ...
            <Image src={logo} alt="logo" objectFit='cover' style={{maxWidth: "50px"}} />
            <p className="text-gray-400 text-base mt-5">The ultimate all-in-one solution for your ecommerce business worldwide.</p>
          </div>
          ...
          <div>
            ...
            {mainMenu.map((menuItem, index) => (
              <li key={index}><Link href={menuItem.link} className="text-gray-400 hover:text-white text-base">{menuItem.name}</Link></li>
            ))}
              </ul>
          </div>
          ...
          <div>
            ...
            <ul className="flex flex-wrap gap-x-5 gap-4">
              <li><Link href="/" className="text-xl">
        ...
        <p className='bg-gray-900 py-3 text-gray-400 text-base text-center'>©2023 Egala Spot All Rights Reserved</p>
      </footer>
    </>
  )
}
